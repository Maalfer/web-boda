...
const Dashboard = () => {
    ...
    const handleDeleteRsvp = async (rowid) => {
        ...
        try {
            const data = await deleteRsvp(rowid);
            if (data.status === 'success') {
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white', fontFamily: 'var(--font-body)' }}>
                    <thead>
                        <tr style={{ backgroundColor: 'var(--color-primary)', color: 'var(--color-text)' }}>
                            ...
                            <th style={{ padding: '1rem', border: '1px solid #ddd' }}>Asistencia</th>
                    ...
                    <tbody>
                        {rsvps.map((rsvp, index) => (
                            <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                                ...
                                <td style={{ padding: '1rem', border: '1px solid #ddd' }}>{rsvp.attendance}</td>
                ...
                <div style={{
                    ...
                    {photos.map((photo) => (
                        <div key={photo.id} style={{
                            ...
                            <a href={getStaticUrl(photo.filename)} target="_blank" rel="noopener noreferrer">
                                <img
                                    src={getStaticUrl(photo.filename)}
                                    alt="Foto subida"
